...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow) {
	...
	wcApp.hIcon = LoadIcon(NULL, /*IDI_APPLICATION*/IDI_ERROR);// "hIcon" = handler do ícon normal
															   //"NULL" = Icon definido no Windows
															   // "IDI_AP..." Ícone "aplicação"
	wcApp.hIconSm = LoadIcon(NULL, /*IDI_INFORMATION*/ IDI_ERROR);// "hIconSm" = handler do ícon pequeno
																  //"NULL" = Icon definido no Windows
	...
	hWnd = CreateWindow(
		...
		TEXT("Titulo Ficha 6 Ex3"),// Texto que figura na barra do título
		WS_OVERLAPPEDWINDOW, // Estilo da janela (WS_OVERLAPPED= normal)
		/*CW_USEDEFAULT*/ 100, // Posição x pixels (default=à direita da última)
		/*CW_USEDEFAULT*/ 100, // Posição y pixels (default=abaixo da última)
		/*CW_USEDEFAULT*/ 640, // Largura da janela (em pixels)
		/*CW_USEDEFAULT*/ 480, // Altura da janela (em pixels)
...
LRESULT CALLBACK TrataEventos(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
	int res;
	static int x = 0, y = 0;
	static TCHAR letras[11] = TEXT("\0");
	//TCHAR text[] = "Text no ecrâ...";

	TCHAR *text1 = "hello world";
	...
	case WM_CREATE:
	{
		...
		bg = CreateSolidBrush(RGB(0, 0, 255));
	...
	case WM_PAINT:
	{
		...
		//Rectangle(hdc, x, y, x + 80, y + 30);
		//TextOut(hdc, x + 5, y + 5, letras, 1);
		//_stprintf_s(letras, sizeof(letras), "(%d, %d)", x, y);
		//TextOut(hdc, x + 5, y + 5, letras, sizeof(letras));
		//EndPaint(hWnd, &ps);
	...
	//case WM_CLOSE:
	//	res = MessageBox(hWnd, "Fechar app?", "confirmar:", MB_YESNO);
	//	switch (res)
	//	{
	//	case IDYES:
	//		DestroyWindow(hWnd);
